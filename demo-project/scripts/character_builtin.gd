...
func _physics_process(_delta) -> void:
	...
	if not self.grounded():
		world_velocity -= Vector3(0, 9.8, 0) * _delta
	elif grounded():
	...
	if grounded():
		move *= Quaternion(self.get_floor_normal(), Vector3.UP)

	self.velocity = move + world_velocity
	self.move_and_slide()
...
func _apply_jump():
	world_velocity = Vector3.UP * jump_velocity
