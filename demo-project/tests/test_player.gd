...
const Player = preload("res://scripts/experimental/character_openkcc_physics_query.gd")
...
func test_player_move(params=use_parameters(move_params)):
	...
	# Character will be accelerating from no velocity, so will be slightly less than move speed
